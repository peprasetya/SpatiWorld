...
class FSViewerManifest:
    ...
    def fs_sign_win_binaries(self):
        ...
        executable_paths = [
            # self.args['configuration'] + "\\spatiworld-bin.exe", # no need to sign this we are not packaging it.
    ...
    def fs_save_windows_symbols(self):
        ...
        pdbName = "spatiworld-bin.pdb"
        try:
            subprocess.check_call( [ "pdbcopy.exe" ,
                                     self.args['configuration'] + "\\spatiworld-bin.pdb", 
                                     self.args['configuration'] + "\\spatiworld-bin-public.pdb",
                                     "-p"
                                 ], stderr=subprocess.PIPE,stdout=subprocess.PIPE )
            pdbName = "spatiworld-bin-public.pdb"
        except:
        ...
        symbolTar.add( "%s/SpatiWorld-bin.exe" % self.args['configuration'].lower(), "spatiworld-bin.exe" )
